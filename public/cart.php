...
if ($remove_id != "non") {
    $sql = "UPDATE invoice set is_delete = 1 where invoice_id = $remove_id";
...
if (isset($_SESSION['id'])) {
    $user_id = $_SESSION['id'];
    $sql = "SELECT * from invoice where done = 0 and is_delete = 0 and user_id = $user_id";
} else {
    $gaccount_id = $_SESSION['google_account'];
    $sql = "SELECT * from invoice where done = 0 and is_delete = 0 and gaccount_id = $gaccount_id";
}
                                    ...
                                    <tbody>
                                        ...
                                        if (mysqli_num_rows($result)) {
                                            while ($rows = mysqli_fetch_assoc($result)) {
                                                ...
                                                $totalPrice += $rows['quantity'] * ($row1['price'] - $row1['discount']);
                                        ?>
                                                <tr>
                                                    <td class="pro-thumbnail"><a href="#"><img class="img-fluid" src="../uploades/<?= $row1['image'] ?>" alt="Product" /></a></td>
                                                    <td style="max-width: 200px;overflow: hidden;" class="pro-title"><a href="product.php?id=<?= $row1['id'] ?>"><?= $row1['name'] ?></a></td>
                                                    <td class="pro-price"><span>$<?= $row1['price'] - $row1['discount'] ?></span></td>
                                                    <td class="pro-price"><span><?= $rows['quantity'] ?></span></td>
                                                    <td class="pro-subtotal"><span>$<?= $rows['quantity'] * ($row1['price'] - $row1['discount']) ?></span></td>
                                                    <td class="pro-remove"><a href="cart.php?remove_id=<?= $rows['invoice_id'] ?>"><i class="fa fa-trash-o"></i></a></td>
                                                </tr>
                                        <?php
                                            }
                                        }
                    ...
                    <div class="row">
                        <div class="col-lg-5 ml-auto">
                            ...
                            <div class="cart-calculator-wrapper">
                                ...
                                <a href="<?php if ($checkout == true) {
                                                echo "checkout.php";
                                            } else {
                                                echo '';
                                            } ?>" class="btn d-block">Proceed Checkout</a>
                            </div>
                        </div>
                    </div>
                </div>
